...
function Choose-ADOrganizationalUnit
{
    ...
    (
	    ...
	    [string]
        $Domain,
	#this function is created by ITMicaH
...
Password"
	...
	$buttonOK_Click={
		...
		{
			New-Variable -Scope 1 -Name SelectedObject -Value $Treeview.SelectedNode.Name
		}
	}
...
function Find-ADUser {
    ...
    $inputBox1.Location = New-Object System.Drawing.Point(100, 20)
    ...
    $inputBox2.Location = New-Object System.Drawing.Point(100, 50)
    $inputBox2.Size = New-Object System.Drawing.Size(150, 20)
    ...
    $form.Controls.Add($cancelButton)
     if ($displayNoUserLabel) {
        $noUserFoundLabel = New-Object System.Windows.Forms.Label
        $noUserFoundLabel.Location = New-Object System.Drawing.Point(10, 110)
        $noUserFoundLabel.Size = New-Object System.Drawing.Size(150, 20)
        $noUserFoundLabel.ForeColor = [System.Drawing.Color]::Red
        $noUserFoundLabel.Text = "No user found"
        $form.Controls.Add($noUserFoundLabel)
    }
   ...
   while ($true) {
        ...
        if ($dialogResult -eq [System.Windows.Forms.DialogResult]::OK) {
            $inputValue1 = $inputBox1.Text
            $inputValue2 = $inputBox2.Text

            $result = [PSCustomObject]@{
                FirstName = $inputValue1
                LastName = $inputValue2
            }
            break  # Break the loop on OK button click
        } elseif ($dialogResult -eq [System.Windows.Forms.DialogResult]::Cancel) {
            $result = $null  # Set result to null on Cancel button click
            break  # Break the loop on Cancel button click
        }
    }
...
function Show-ADUserListView {
    ...
    $listView.Columns.Add('User') | Out-Null
    $listView.Columns.Add('Employee ID') |Out-Null
    $listView.Columns.Add('E-Number') | Out-Null

    $listView.Columns[0].Width = 134
    $listView.Columns[1].Width = 133
    $listView.Columns[2].Width = 133

    # Add items to the ListView from the provided Get-ADUser array
    foreach ($user in $ADUsers) {
        $item = New-Object System.Windows.Forms.ListViewItem($user.CN)
        
        # Check if EmployeeID exists and is not null/empty before adding
        if ($user.EmployeeID) {
        ...
        # Check if EmployeeNumber exists and is not null/empty before adding
        if ($user.EmployeeNumber) {
            $item.SubItems.Add($user.EmployeeNumber)
        } else {
            $item.SubItems.Add("<empty>")
        }
    ...
    $changeButton.Location = New-Object System.Drawing.Point(10, 260)
    $changeButton.Size = New-Object System.Drawing.Size(120, 30)
    $changeButton.Text = "Change Details"
    $changeButton.Add_Click({
        # Retrieve the selected AD user's CN when the button is clicked
        if ($listView.SelectedItems.Count -gt 0) {
            $selectedCN = $listView.SelectedItems[0].Text
            Write-Output "Selected User CN: $selectedCN"
            
            # Retrieve Employee ID and Employee Number
            $employeeDetails = Get-EmployeeDetails -EmployeeID $listView.SelectedItems[1].Text -EmployeeNumber $listView.SelectedItems[2].Text
            if ($employeeDetails -ne $null) {
                # Update AD user details
                Set-ADUserDetails -CN $selectedCN -EmployeeID $employeeDetails.EmployeeID -EmployeeNumber $employeeDetails.EmployeeNumber -OU $OU -DomainName $DomainName
            }
        } else {
            Write-Output "Please select a user from the list."
        }
    })
...
function Set-ADUserDetails {
    ...
    # Fetch the user based on CN
    $user = Get-ADUser -Filter "CN -eq '$CN'" -Server $DomainName -SearchBase $OU -Properties EmployeeID,EmployeeNumber

    if ($user) {
        # Update EmployeeID and EmployeeNumber if they are provided
        Write-Host "Set-ADUserDetails in"
        if ($EmployeeID) {
            Set-ADUser -Identity $user -Server $DomainName -Credential $credentials -EmployeeID $EmployeeID 
        }

        if ($EmployeeNumber -ne "") {
            Set-ADUser -Identity $user -Server $DomainName -Credential $credentials -EmployeeNumber $EmployeeNumber
        }

        Write-Output "User details updated successfully:"
        Write-Output "User CN: $($user.CN)"
        Write-Output "Employee ID: $($user.EmployeeID)"
        Write-Output "Employee Number: $($user.EmployeeNumber)"
    } else {
        Write-Output "User with CN '$CN' not found."
    }
}
...
function Search-ADUsers {
    ...
    if (-not [string]::IsNullOrWhiteSpace($firstName) -and -not [string]::IsNullOrWhiteSpace($lastName)) {
        ...
        $filter = {GivenName -like $firstName -and Surname -like $lastName}
    } elseif (-not [string]::IsNullOrWhiteSpace($firstName)) {
        # Search by first name only
        $filter = {GivenName -like $firstName}
    } elseif (-not [string]::IsNullOrWhiteSpace($lastName)) {
        # Search by last name only
        $filter = {Surname -like $lastName}
    } else {
        # No parameters provided for the search
        Write-Output "Please provide at least a first name or last name for the search."
        return $null
    }

    # Search for users based on constructed filter
    $users = Get-ADUser -Filter $filter -Server $DomainName -SearchBase $OU -Properties CN,EmployeeID,EmployeeNumber
...
function Get-EmployeeDetails {
    ...
    # Create the form
    $form = New-Object System.Windows.Forms.Form
    $form.Text = "Employee Details"
    $form.Width = 300
    $form.Height = 200
    $Form.StartPosition = "CenterScreen"

    # Create a label and text box for Employee ID
    ...
    $inputBoxID = New-Object System.Windows.Forms.TextBox
    $inputBoxID.Location = New-Object System.Drawing.Point(10, 50)
    $inputBoxID.Size = New-Object System.Drawing.Size(260, 20)
    $inputBoxID.Text = $EmployeeID  # Pre-filling Employee ID
    $form.Controls.Add($inputBoxID)

    # Create a label and text box for Employee Number
    $labelNumber = New-Object System.Windows.Forms.Label
    $labelNumber.Location = New-Object System.Drawing.Point(10, 80)
    $labelNumber.Size = New-Object System.Drawing.Size(280, 20)
    $labelNumber.Text = "Employee Number:"
    $form.Controls.Add($labelNumber)

    $inputBoxNumber = New-Object System.Windows.Forms.TextBox
    $inputBoxNumber.Location = New-Object System.Drawing.Point(10, 110)
    $inputBoxNumber.Size = New-Object System.Drawing.Size(260, 20)
    $inputBoxNumber.Text = $EmployeeNumber  # Pre-filling Employee Number
    $form.Controls.Add($inputBoxNumber)
    ...
    $cancelButton = New-Object System.Windows.Forms.Button
    $cancelButton.Location = New-Object System.Drawing.Point(180, 140)
    $cancelButton.Size = New-Object System.Drawing.Size(75, 23)
    $cancelButton.Text = "Cancel"
    $cancelButton.DialogResult = [System.Windows.Forms.DialogResult]::Cancel
    $form.Controls.Add($cancelButton)
    ...
    # Check if OK was clicked and retrieve the input
    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
        $inputValueID = $inputBoxID.Text
        $inputValueNumber = $inputBoxNumber.Text
        return @{
            EmployeeID = $inputValueID
            EmployeeNumber = $inputValueNumber
        }
    } else {
        return $null
    }
...
$DomainName = "Input Domain Controller Name"
...
write-host $username
write-host $password
$password0 = ConvertTo-SecureString $password -AsPlainText -Force

#get credentials
$credentials = New-Object System.Management.Automation.PSCredential -ArgumentList $username, $password0 
$session = New-PSSession -ComputerName $DomainName -Credential $credentials

#Designate the OU you wish to get into
#add your domain to DC
    $OU = Choose-ADOrganizationalUnit -Domain "DC=Basic,DC=local" -AdvancedFeatures
...
while ($continue) {
    ...
    if ($result -ne $null) {
        ...
        $users = Search-ADUsers -firstName $firstName -lastName $lastName

        if ($users -ne $null) {
            Write-Output "Users found:"
            Write-Output $users
            $userArrayList = New-Object System.Collections.ArrayList
            # Assuming $users contains the ADUser objects retrieved from Active Directory
            foreach ($user in $users) {
                [void]$userArrayList.Add($user)
            }
            
            Show-ADUserListView -ADUsers $userArrayList
            

        } else {
            Write-Output "No users found with the provided parameters."
        }
    } else {
        $continue = $false  # Set $continue to false when the user cancels
    }
}
